...
def naive_to_utc(naive_dt: dt.datetime, timezone: pytz.timezone):
    """
    Transform datetimes that do not have a timezone specified to UTC
    ...
    if naive_dt.tzinfo is None:
        return timezone.localize(naive_dt, is_dst=None).astimezone(pytz.UTC)
    else:
...
def utc_to_local(utc_dt: dt.datetime, local_timezone: pytz.timezone) -> dt.datetime:
    ...
    :return: A datetime obejct that reflects the same moment as `utc_dt`, converted to the specified timezone.
    """

    local_dt = utc_dt.replace(tzinfo=pytz.utc).astimezone(local_timezone)
    return local_timezone.normalize(local_dt)

def local_to_utc(local_dt: dt.datetime):
    """
